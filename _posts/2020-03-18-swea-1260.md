...
정답을 구하려면 첫 행렬 부터 마지막 행렬까지 계산했을 때의 최소 곱셈 연산 횟수이다. 이를 위해 solve(p1, p2)라는 함수를 생각해보자. 

solve(p1, p2)는 [p1 - p2] 범위 사이를 두 덩어리로 나누어 행렬곱을 수행한다.
...
때문에, 시간이 오래걸리므로 dp를 이용하여 한번 계산한 결과를 저장하도록하자.

	dp[처음][끝] = 처음~끝 범위에서 행렬곱을 수행하였을 때 최소 계산 횟수
