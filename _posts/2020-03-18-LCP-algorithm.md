...
하지만 이와같은 방법은 정답을 찾을 수 있지만 모든 부분 문자열을 찾은 후 그에 대한 정렬까지 해주어야 하기 때문에 퍼포먼스가 낮게 나온다. 따라서, `접미사(Suffix)`를 이용한 방법을 통해 구하였다.
...
이렇게 정렬된 접미사 각각 앞에서 하나씩 잘라서 나열하면 모든 부분문자열을 정렬한 것과 같다.
|index|SA|부분|
|-|-|:-|
|3|d|d|
|0|food|f|
